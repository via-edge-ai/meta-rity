...
EOC
	...
	if [ "${1}" = "u-boot-initial-env" ]; then
	        ...
	        /bin/echo -e 'mmc_boot=mmc dev \x24{devnum}; part start mmc \x24{devnum} kernel spa; part size mmc \x24{devnum} kernel sps; mmc read \x24{scriptaddr} \x24{spa} \x24{sps}; source \x24{scriptaddr}' >> ${DEPLOYDIR}/u-boot-initial-env
	        /bin/echo -e 'scsi_boot=run scsi_init; scsi dev \x24{devnum}; part start scsi \x24{devnum} kernel spa; part size scsi \x24{devnum} kernel sps; scsi read \x24{scriptaddr} \x24{spa} \x24{sps}; source \x24{scriptaddr}' >> ${DEPLOYDIR}/u-boot-initial-env
	fi
}
